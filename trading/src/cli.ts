...
import { getOpenMarkets, getTradingMarket, isRegistryAvailable, type TradingMarket } from "./registry/index.js";
...
`);
  ...
  for (const agent of agents) {
    console.log(`  ✓ @${agent.handle} (${agent.personality.riskProfile}, ${agent.personality.tradingStyle})`);
  }
  ...
  for (const i of interested) {
    console.log(`    @${i.agentId.replace('agent_', '')}: ${i.side} ${i.quantity} ${i.token} @ $${i.price?.toFixed(2)}`);
  ...
  for (let round = 1; round <= 10; round++) {
    ...
    const activeChats = Array.from(sideChats.values()).filter(c => c.status === "NEGOTIATING");
    if (activeChats.length > 0) {
      console.log(`\n  [Running ${activeChats.length} active side chat(s)]`);
      for (const chat of activeChats) {
        console.log(`  [Side Chat: ${chat.participants.map(p => '@' + p.replace('agent_', '')).join(' ↔ ')}]`);
    ...
    const agentsWhoLeft = floorMessages.filter(m => m.type === "AGENT_LEFT").map(m => m.agentId);
...
`);
  ...
  if (!registryOk) {
    console.error("[Trading] ❌ Registry not available. Start it with: pnpm registry");
  ...
  for (const agent of agents) {
    console.log(`  ✓ @${agent.handle} ($10,000) - ${agent.personality.riskProfile}`);
  }
  ...
  while (true) {
    ...
    if (newMarkets.length > 0) {
      ...
      console.log("[Trading] No open markets found. Waiting for new markets...");
    }
...
`);
  ...
  for (let round = 1; round <= 10; round++) {
    ...
    const agentsLeft = floorMessages.filter(m => m.type === "AGENT_LEFT").length;
    if (agentsLeft >= 5) {
      console.log("\n  All agents have left the floor.\n");
      break;
    }
