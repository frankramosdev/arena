...
function getMessageTypeInfo(type: string) {
  switch (type) {
    ...
    case "RFQ":
      return { label: "📢 RFQ", color: "#B45309", bg: "var(--amber-soft)" };
    case "TRADE":
      return { label: "✅ TRADE", color: "#166534", bg: "var(--green-soft)" };
    case "AGENT_LEFT":
      return {
        label: "👋 LEFT",
          ...
          <div className="space-y-2">
            {activeSessions.map((session) => (
              <button
                ...
                style={{
                  background:
                    selectedMarket === session.marketId
                      ? "var(--accent-primary)"
                      : "var(--bg-card)",
                  color:
                    selectedMarket === session.marketId
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">
                      {shortMarket(
                        session.marketQuestion || session.marketId,
                        35,
                      )}
                    </p>
...
function ChatMessage({
  ...
  const agentHandle =
    message.agentHandle || message.agentId?.replace("agent_", "") || "system";
...
function TradeCard({
  ...
  trade: TradingTrade & { marketQuestion?: string };
}) {
    ...
    >
      ...
      <div className="flex items-center justify-between text-xs">
        <span style={{ color: "var(--text-secondary)" }}>
          @{trade.buyer.replace("agent_", "")} ← @
          {trade.seller.replace("agent_", "")}
        </span>
...
export default function TradingFloorPage() {
  ...
  const [agentMap, setAgentMap] = useState<Map<string, TradingAgent>>(
    new Map(),
  );
  const [messages, setMessages] = useState<
    (TradingMessage & { marketQuestion?: string })[]
  >([]);
  const [trades, setTrades] = useState<
    (TradingTrade & { marketQuestion?: string })[]
  >([]);
  ...
  const filteredTrades = trades.filter(
    (t) => !selectedMarket || t.marketId === selectedMarket,
  );
                      ...
                      <div>
                        ...
                        >
                          {selectedMarket && selectedSession
                            ? shortMarket(
                                selectedSession.marketQuestion ||
                                  selectedMarket,
                                50,
                              )
                            : "All Activity"}
                        ...
                        >
                          ...
                          {selectedSession &&
                            ` · Round ${selectedSession.stats.currentRound || 1}/${selectedSession.stats.maxRounds || 10}`}
                        </p>
                      </div>
                    </div>
